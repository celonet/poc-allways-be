...
describe('Integration tests', () => {
  describe('Repository', () => {
    ...
    test('Should to update one data in the database', async () => {
      ...
      const recordUpdated = await card.updateOne({ _id: record._id }, { subject: 'Subject Test updated' }, { new: true })
    ...
    test('Should to update three data in the database', async () => {
      const recordsUpdated = await card.updateMany({ description: cardMock.description },
        { updated_at: new Date().toISOString() }, { multi: true })

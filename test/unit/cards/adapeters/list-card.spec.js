...
const list = require('../../../../v1/adapters/cards/list')
...
describe('Unit tests', () => {
  const mocks = {
    ...
    onSuccess: ({ cards, total }) => {
      if (cards.length > 5) {
          const error = new Error('Invalid object of cards')
          error.name = 'InvalidObjectCards'
          throw error
      }
      return { cards, total }
    },
  ...
  describe('List of Cards', () => {
    ...
    test('Should to return an error, Invalid Object Card', async () => {
      ...
      mock.repository.findMany = async () => [...listStub, listStub[0]]
      ...
      expect(response.name).toEqual('InvalidObjectCards')
